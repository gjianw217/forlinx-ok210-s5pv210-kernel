...
dm9000_probe(struct platform_device *pdev)
{
	...
	/* set timing */
	srom_reg_base = ioremap(0xE8000000, 0x8);
	if (!srom_reg_base) {
	...
	bw = readl_relaxed(srom_reg_base);
	bw &= 0xFFFFFFF0;
	bw |= 0x00000003; // byte base address, 16bit
	writel_relaxed(bw, srom_reg_base);
	...
	writel_relaxed(cs, srom_reg_base + 0x4);
	dev_info(dev, "set dm9000 srom timing\n");
